...
void Monster::InitMonsterSprite(char *name, char *xue_back, char* xue_fore)  //带血条的怪物  函数重载
{
	...
	Monster_xue->setPosition(ccp(m_MonsterSprite->getPositionX() + 25, m_MonsterSprite->getPositionY() + 50));//设置在怪物上头    																											  //Monster_xue->setScale(2.2f);    
...
void  Monster::SetAnimation(const char *name_each, unsigned int num, bool run_directon)
{
	if (MonsterDirecton != run_directon)
	{
		...
		m_MonsterSprite->setFlipX(run_directon);//增加！
	}
...
void Monster::FollowRun(Hero* m_hero, CCNode* m_map)
{
	...
	if (x<-64)//判断怪物横坐标和英雄的距离  
	{
		...
		m_MonsterSprite->setFlipX(MonsterDirecton);//设置方向  增加！
		if (IsAttack)
			return;

...
bool SocketServer::initServer(unsigned short port)//根据端口号初始化服务端
{
	...
	do
	{
		...
		Director::getInstance()->getScheduler()->scheduleUpdate(this, 0, false);//尝试模仿客户端使其也能接收消息
...
void SocketServer::update(float dt)
{
	...
	case RECEIVE:
		if (onRecv)
		{
			...
			//this->onRecv(msg->getMsgType, (const char*)msg->getMsgData()->getBytes(), msg->getMsgData()->getSize());
		}
		break;
	default:
		break;
